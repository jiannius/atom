...
- **Trashed:** add the `trashed` prop to `<atom:table>` to append an icon toggle (tooltip "Show archived") at the end of the header bar — it drives `$_table.show_trashed` and `toTable()` applies `onlyTrashed()`. `<atom:table.trashed />` is the standalone component for custom placement.
