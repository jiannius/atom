<div class="image-menu bg-white dark:bg-zinc-700 rounded-lg shadow-sm border border-zinc-200 dark:border-zinc-600" x-init="$el.shouldShow = (editor) => (editor?.isActive('image'))">
    <div class="flex items-center flex-wrap p-1">
        ...
        @foreach (['30%', '50%', '80%', '100%'] as $size)
            <atom:tiptap.menu.button x-on:click="commands().updateAttributes('image', { width: {{ js($size) }} })">
        ...
        <atom:tiptap.menu.button label="Remove" x-on:click="commands().deleteSelection()">
